...
\begin{document}
	...
	\begin{proof}
		...
		such that the following diagram commutes.
		\begin{centikzcd}
			U(X^*)%\ar[rr,bend left,"\exists! U(h)"]
			\ar[r,
			%tail,two heads,
		...
		%Let $g\colon i_B(X)\iso u(X)$ be an isomorphism,
		%Let $g\colon B'\iso A'$ be an isomorphism,
		%then we can define $h\colon A\to A$ as:
		%\[
		%h(a)=\begin{cases}
		%	a & (a\notin i_B(X)\symdif u(X))\\
		%	g(a) & (a\in i_B(X)\setminus u(X))\\
		%	g^{-1}(a) & (a\in u(X)\setminus i_B(X) )
		%\end{cases}.
		%\]
		%Easy to show that $h$ is involution, \ie, $h\circ h=1_A$.
		Let $g\colon A'\iso B'$ be a bijection in $\mathbf{Sets}$, %an isomorphism,
		...
		%We can then define monoid $C$, whose underlying set is $A$,
		%equipped with multiplication:
		%\[ a \cdot_C b \define h(a)\cdot_B h(b)\in A=U(C). \]
		%
		%The
	...
	\end{proof}
	%\begin{rem*}
	%	Note that this is generally not true if $X$ is infinite.
	%\end{rem*}
	\begin{rem*}
