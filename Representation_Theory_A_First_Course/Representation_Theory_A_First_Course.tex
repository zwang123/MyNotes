...
% For example, the action of the symmetric group of X is transitive, the action of the general linear group or the special linear group of a vector space V on V ∖ {0} is transitive, but the action of the orthogonal group of a Euclidean space E is not transitive on E ∖ {0} (it is transitive on the unit sphere of E, though).
	...
	\begin{proof}
		...
		which always exists due to \ref{thmHilbertOrthonormal},
	...
	\end{def*}
	\begin{rem*}
		In total, a Lie group is a tuple ($K$ is either $\re$ or $\co$)
		...
		\forall i\in I, \forall (V,\psi\colon V\to K^{n'})$):
		\begin{itemize}
			...
			\begin{eqlong}
				...
				&(V,\psi\colon V\to K^{n'})\in \mathcal{A} \iff\\
				&\Big( V\in\tau \land \forall p,q \in V 
				%\forall \alpha,\beta \in \psi(V) 
				\big( p=q\iff\psi(p)=\psi(q) \big) \\
				&\land \forall S \subseteq V 
				\big( S\in\tau\iff\psi(S)\in \mathcal{H}(K^{n'}) \big)\\
				&\land (\psi \circ \varphi_i^{-1}) \text{ and } (\varphi_i \circ \psi^{-1}) \text{ are smooth/holomorphic}
				 \Big);
				 \\
			\end{eqlong}
			...
			\begin{eqlong}
				&\exists j,k,l\in I\bigg[ x\in U_j\land y\in U_k\land (x)^{-1}\cdot y\in U_l\land\\
				&\Big( \varphi_l \circ \big( (p,q)\mapsto (p)^{-1}\cdot q \big) \circ (\varphi_j\times\varphi_k)^{-1} \Big)
				\text{ is smooth/holomorphic}
				\bigg],\\
			\end{eqlong}
		\end{itemize}
		where $\mathcal{H}(K^{n})$ retrieves the topology on a finite-dimensional Hilbert space $K^{n}$ (\ref{propEuclideanTopology}).
	\end{rem*}
	\begin{rem*}
