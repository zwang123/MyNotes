		...
		\begin{tikzpicture}	
			\node[scale=.8] (a) at (0,0){
				\begin{tikzcd}[anchor=center,
					...
					\ar[d]\ar[ld,tail,"\text{PC/L/(M+AC)}"{sloped}']&\text{Metrizable} \ar[from=l,leftrightarrow,dashed,"\text{Complete + TB}"]
					\ar[d,tail,"\text{Manifold}"]\\
					%%%%%%%%%%%%%%%
					\text{Pseudocompact} \ar[from=r,tail,"T_4"']\ar[from=ur,tail,"M"']
					&\text{Countably Compact}\ar[d,tail,"\text{M+AC}"']&\text{Hemicompact}\ar[d]&\text{Paracompact}\\
					%%%%%%%%%%%%%%%
					&\text{Limit Point Compact}&\sigma\text{-compact}\ar[d]&\text{Mesocompact}\ar[from=u]\ar[d] &[-4cm]T_4\ar[from=ul,dashed,"T_2"]\\
					%%%%%%%%%%%%%%%
					\text{Separable}&\textbf{Second-Countable}\ar[r,tail,"\text{M}"]\ar[l,tail,"\text{M}"'] &\text{Lindelöf}\ar[from=uu,leftrightarrow,bend left=50,dashed,"\text{LC}"]\ar[ruu,dashed,pos=0.7,"\text{Regular}"{sloped}]
