...
Theorem. Any complete inner product space has an orthonormal basis.
	...
	\paragraph{More on Metric Space}
	...
	Metric spaces are first countable since one can use balls with rational radius as a neighbourhood base.
	
	%The metric topology on a metric space {\displaystyle M}M is the coarsest topology on {\displaystyle M}M relative to which the metric {\displaystyle d}d is a continuous map from the product of {\displaystyle M}M with itself to the non-negative real numbers.
		...
		\begin{tikzpicture}	
			\node[scale=.8] (a) at (0,0){
				\begin{tikzcd}[anchor=center,
					...
					%%%%%%%%%%%%%%%
					&\text{Sequentially Compact}\ar[r,leftrightarrow,dashed,"\text{M + AC}{}_\omega"]&\text{Compact}\ar[rd]
					\ar[d,tail,"\text{CC}"]\ar[ld,tail,"\text{PC/L/(M+AC)}"']&\text{Metric} \ar[from=l,leftrightarrow,dashed,"\text{Complete + TB}"]\ar[d]\\
					%%%%%%%%%%%%%%%
					\text{Pseudocompact} \ar[from=r,tail,"T_4"]\ar[from=ur,tail,"M"']
					&\text{Countably Compact}\ar[d,tail,"\text{M+AC}"]&\text{Lindelöf}\ar[r,dashed,"\text{Regular}"]&\text{Paracompact}\\
					%%%%%%%%%%%%%%%
					&\text{Limit Point Compact}&T_4\ar[from=ur,dashed,"T_2"]&\text{Mesocompact}\ar[from=u]\ar[d] \\
					&&&\text{Metacompact}\ar[d]\\
					&&&\text{Orthocompact}\\
				\end{tikzcd}
			};
		\end{tikzpicture}
		\caption{Hierarchy of compactness: solid line = unconditional, dashed line = conditional, tailed arrow = unconditional + conditional converse\\
			M = metric space, L = Lindelöf, CC = countably compact, PC = paracompact, AC = axiom of choice, TB = totally bounded
		}
...
In a metric space, the notions of sequential compactness, limit point compactness, countable compactness and compactness are all equivalent (if one assumes the axiom of choice).
...
A manifold is second-countable iff it is separable and paracompact.
