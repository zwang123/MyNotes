...
% For example, the action of the symmetric group of X is transitive, the action of the general linear group or the special linear group of a vector space V on V ∖ {0} is transitive, but the action of the orthogonal group of a Euclidean space E is not transitive on E ∖ {0} (it is transitive on the unit sphere of E, though).
	...
	\end{def*}
	%\begin{thm}
	%	The ideal $I$ of a commutative ring $R$ is prime iff $R/I$ is an integral domain.
	%\end{thm}
	%\begin{thm}
	%	Maximal ideal of a commutative ring is prime.
	%\end{thm}
	...
	\begin{thm}
		...
		\begin{eqlong}
			...
			%\operatorname{rad}(\set{0};R)=\set{0}&\land \operatorname{Spec}(R)\text{ disconnected}\\
			%\implies
			%\exists R_1,R_2& [R\cong R_1\times R_2\land \operatorname{Spec}(R)
			%\cong\operatorname{Spec}(R_1)\sqcup\operatorname{Spec}(R_2)] ,\\
